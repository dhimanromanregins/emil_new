...
export default function Products() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get('http://20.235.152.209/api/products/');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://20.235.152.209/api/products/', formData, {
        headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`http://20.235.152.209/api/products/${productId}`);
      // Show toast message
  ...
  return (
    ...
    <div className="add-product-container mt-4">
      ...
      <form onSubmit={handleSubmit} className="add-product-form" encType="multipart/form-data">
        ...
        <div className='Addproduct'><button type="submit" className="btn">Add Product</button></div>
