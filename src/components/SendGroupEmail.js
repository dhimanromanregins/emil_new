...
export default function SendGroupEmail() {
    ...
    useEffect(() => {
      ...
      axios.get('http://20.235.152.209/api/send-group-email/')
        .then(response => {
    ...
    const handleSendGroupEmail = () => {
      ...
      setIsSending(true);
      axios.post('http://20.235.152.209/api/send-group-email/', { group_name: selectedGroup })
        .then(response => {
      ...
      <>
        ...
        <div className="send-bulk-email-container">
          ...
          <div className="select-container">
            <select className='jhfioj'
              value={selectedGroup}
              onChange={handleSelectChange}
            >
              <option className='ghhl' value="">Select Group</option>
